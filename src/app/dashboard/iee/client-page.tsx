...
interface ClientIEEEPageProps {
    initialData: IEEERecord[];
    total: number;
}
...
export default function ClientIEEEPage({ initialData, total }: ClientIEEEPageProps) {
    ...
    const [selectedItem, setSelectedItem] = useState<IEEERecord | null>(null);
    const [formData, setFormData] = useState({ mebership_id: '', validity: false });
    const [error, setError] = useState('');
    const [isLoading, setIsLoading] = useState(false);

    const columns: { key: keyof IEEERecord; label: string; sortable?: boolean }[] = [
    ...
    const openEditModal = (item: IEEERecord) => {
        setSelectedItem(item);
        setFormData({ mebership_id: item.mebership_id, validity: item.validity });
        setIsEditOpen(true);
    };

    const openDeleteModal = (item: IEEERecord) => {
        setSelectedItem(item);
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-400 mt-1">Manage IEEE membership records</p>
                </div>
                <Button
                    onClick={() => {
                        setFormData({ mebership_id: '', validity: false });
            ...
            <Modal
                ...
                title="Create IEEE Record"
                description="Add a new IEEE membership record"
            >
                {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
                <div className="space-y-4">
                    <div>
