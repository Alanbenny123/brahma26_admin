...
import { deleteIEE, createIEE, updateIEE } from "@/actions/appwrite";
...
interface ClientIEEPageProps {
    initialData: IEERecord[];
    total: number;
}

export default function ClientIEEPage({ initialData, total }: ClientIEEPageProps) {
    const [isDeleteOpen, setIsDeleteOpen] = useState(false);
    const [isCreateOpen, setIsCreateOpen] = useState(false);
    const [isEditOpen, setIsEditOpen] = useState(false);
    const [selectedItem, setSelectedItem] = useState<IEERecord | null>(null);
    const [formData, setFormData] = useState({ mebership_id: '', validity: false });
    const [error, setError] = useState('');
    const [isLoading, setIsLoading] = useState(false);

    const columns: { key: keyof IEERecord; label: string; sortable?: boolean }[] = [
        { key: "membership_id", label: "Membership ID", sortable: true },
    ...
    const handleDelete = async () => {
        ...
        setError('');
        try {
            await deleteIEE(selectedItem.$id);
    ...
    const handleUpdate = async () => {
        ...
        setError('');
        try {
            await updateIEE(selectedItem.$id, {
                mebership_id: formData.mebership_id.trim(),
                validity: formData.validity,
            });
            setIsEditOpen(false);
    ...
    const openEditModal = (item: IEERecord) => {
        setSelectedItem(item);
        setFormData({ mebership_id: item.mebership_id, validity: item.validity });
        setIsEditOpen(true);
    };

    const openDeleteModal = (item: IEERecord) => {
        setSelectedItem(item);
        setIsDeleteOpen(true);
    };

    const handleToggleValidity = async (item: IEERecord) => {
        setIsLoading(true);
        try {
            await updateIEE(item.$id, {
        ...
        <div className="p-8 space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-400 mt-1">Manage IEE membership records</p>
                </div>
                <Button
                    onClick={() => {
                        setFormData({ mebership_id: '', validity: false });
            ...
            <Modal
                ...
                title="Create IEE Record"
                description="Add a new IEE membership record"
            >
                {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
                <div className="space-y-4">
                    <div>
