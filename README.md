...
// Default: Fetch up to 1000 documents (fast)
...
// Fetch ALL data using pagination (complete dataset)
const { users: allUsers, total: allTotal } = await fetchUsers(true);
console.log(`Fetched ALL ${allTotal} users`);
...
**Fetch Modes:**
- `fetchUsers()` - Default mode: Returns up to 1000 documents (faster)
- `fetchUsers(true)` - Fetch All mode: Returns ALL documents via pagination (complete)

Same applies to all fetch functions: `fetchEvents()`, `fetchTickets()`, `fetchTransactions()`, `fetchAttendance()`
